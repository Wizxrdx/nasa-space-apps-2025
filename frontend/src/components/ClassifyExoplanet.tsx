...
export default function ClassifyExoplanet({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const resp = await fetch('https://api.wizxrdx.me/api/v1/models/all/', { signal: ac.signal });
  ...
  async function handleClassify() {
    ...
    try {
      ...
      const resp = await fetch(`https://api.wizxrdx.me/api/v1/predict/?${qs.toString()}`, { method: 'POST', body: form });
      if (!resp.ok) {
